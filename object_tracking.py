...
video = cv2.VideoCapture("footballmove.mp4")
...
def goal_track(img, bbox):
    ...
    cv2.circle(img, (c1, c2), 2, (0, 0, 255), 1)
    cv2.circle(img, (int(p1), int(p2)), 2, (0, 255, 0), 1)

    dist = math.sqrt(((c1-p1)**2) + (c2-p2)**2)

    if (dist <= 20):
        cv2.putText(img, "Goal Reached, Tracjectory plotted.",
                    (300, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    xs.append(c1)
    ys.append(c2)
    for i in range(len(xs)-1):
        cv2.circle(img, (xs[i], ys[i]), 2, (0, 0, 255), 1)
